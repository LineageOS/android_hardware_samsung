...
Return<void> Power::setInteractive(bool interactive) {
    ...
    if (!sec_touchkey.empty()) {
        ...
        if (!interactive) {
             ...
             * from suspend.
             */
            if (button_state == 0) touchkeys_blocked = true;
        }
        if (!touchkeys_blocked) set(sec_touchkey, interactive ? "1" : "0");
    }
...
Return<void> Power::powerHint(PowerHint hint, int32_t data) {
    ...
    switch (hint) {
        ...
        default:
            ...
            if (hint == static_cast<PowerHint>(LineagePowerHint::CPU_BOOST)) sendBoost(data);
...
void Power::initialize() {
    ...
    for (const std::string& interactivePath : cpuInteractivePaths)
        hispeed_freqs.emplace_back(get(interactivePath + "/hispeed_freq", std::string()));

    for (const std::string& sysfsPath : cpuSysfsPaths)
        max_freqs.emplace_back(get(sysfsPath + "/cpufreq/scaling_max_freq", std::string()));

    initialized = true;
}

void Power::findInputNodes() {
    for (auto& de : std::filesystem::directory_iterator("/sys/class/input/")) {
        /* we are only interested in the input devices */
        if (std::string(de.path()).find("/sys/class/input/input") == std::string::npos) continue;

        for (auto& de2 : std::filesystem::directory_iterator(std::string(de.path()).c_str())) {
            if (std::string(de2.path()).find("/name") != std::string::npos) {
                std::string content = get(de2.path(), std::string());
                if (content == "sec_touchkey") {
                    sec_touchkey = std::string(de.path()).append("/enabled");
                    LOG(INFO) << "found sec_touchkey: " << sec_touchkey;
                } else if (content == "sec_touchscreen") {
                    sec_touchscreen = std::string(de.path()).append("/enabled");
