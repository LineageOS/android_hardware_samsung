...
void Power::findInputNodes() {
    for (auto& de : std::filesystem::directory_iterator("/sys/class/input/")) {
        ...
        if (de.path().string().find("/sys/class/input/input") == std::string::npos) continue;

        for (auto& de2 : std::filesystem::directory_iterator(de.path())) {
            if (de2.path().string().find("/name") != std::string::npos) {
