...
Return<bool> LineagePowerShare::isEnabled() {
    return get(POWERSHARE_PATH, 0) == 1;
}

Return<bool> LineagePowerShare::setEnabled(bool enabled) {
    set(POWERSHARE_PATH, enabled ? 1 : 0);

    return isEnabled();
}

Return<uint32_t> LineagePowerShare::getMinBattery() {
    return get(POWERSHARE_STOP_CAPACITY_PATH, 0);
}

Return<uint32_t> LineagePowerShare::setMinBattery(uint32_t minBattery) {
