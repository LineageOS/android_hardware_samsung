...
struct BiometricsFingerprint : public ISecBiometricsFingerprint {
    ...
    Return<void> request(int32_t cmd_id, int32_t outBuf_len, int32_t inParam,
                         const hidl_vec<int8_t>& inputBuf, request_cb _hidl_cb) override;
  ...
  private:
    ...
    int (*ss_fingerprint_request)(int32_t cmd_id, const int8_t* inputBuf, uint32_t value,
                                  int8_t* outBuf, uint32_t len, uint32_t inParam);
};
