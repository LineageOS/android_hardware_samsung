...
Light::Light() {
    ...
    if (!mBacklight.is_open())
        LOG(ERROR) << "Failed to open " << PANEL_BRIGHTNESS_NODE << " " << strerror(errno);
    mButtonlight.open(BUTTON_BRIGHTNESS_NODE);
    if (!mButtonlight.is_open())
        LOG(ERROR) << "Failed to open " << BUTTON_BRIGHTNESS_NODE << " " << strerror(errno);
    mIndicator.open(LED_BLINK_NODE);
    if (!mIndicator.is_open())
        LOG(ERROR) << "Failed to open " << LED_BLINK_NODE << " " << strerror(errno);
#ifdef LED_BLN_NODE
    mBacklightNotification.open(LED_BLN_NODE);
    if (!mBacklightNotification.is_open())
        LOG(ERROR) << "Failed to open " << LED_BLN_NODE << " " << strerror(errno);
#endif
}
...
Return<Status> Light::setLight(Type type, const LightState& state) {
    ...
    switch (type) {
        ...
        case Type::ATTENTION:
            // somehow this is flaky, try to open it again
            if (!mIndicator.is_open())
                mIndicator.open(LED_BLINK_NODE);
...
#ifdef LED_BLN_NODE
                if (mBacklightNotification.good() && type == Type::NOTIFICATIONS) {
                    mBacklightNotification << (state.color & COLOR_MASK) ? 1 : 0 << std::endl;
                }
#endif
