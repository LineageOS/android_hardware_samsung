...
void Light::setNotificationLED() {
    ...
    std::stringstream ss;
    ss << std::hex << "0x" << std::setfill('0') << std::setw(8) << state.color << std::setw(0)
       << " " << state.flashOnMs << " " << state.flashOffMs;

    LOG(INFO) << "color=" << std::hex << "0x" << std::setfill('0') << std::setw(8) << state.color
              << std::setw(0) << " delay_on=" << state.flashOnMs
              << " delay_off=" << state.flashOffMs << " flashMode=" << (uint32_t)state.flashMode;
