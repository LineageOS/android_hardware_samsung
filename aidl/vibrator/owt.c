...
static int parse_float(char* frac, int* result, int scale, float min, float max) {
    ...
    if (fres < min || fres > max) return -ERANGE;
...
static int dspmem_chunk_write(struct dspmem_chunk* ch, int nbits, uint32_t val) {
    ...
    if (ch->cachebits == 24) {
        if (dspmem_chunk_end(ch)) return -ENOSPC;
    ...
    if (nbits) return dspmem_chunk_write(ch, nbits, val);
...
int owt_upload(uint8_t* data, uint32_t num_bytes, int gpi, int fd, bool edit,
    ...
    effect->u.periodic.custom_data =
            (int16_t*)malloc(sizeof(int16_t) * effect->u.periodic.custom_len);
...
static int wt_type10_comp_to_buffer(struct wt_type10_comp* comp, int size, void* buf) {
...
static int wt_type10_comp_waveform_get(char* str, struct wt_type10_comp_section* section) {
    ...
    if (strlen(str) < 3) return -EINVAL;

    if (isalpha(str[i++]) && isalpha(str[i++]) && isalpha(str[i++])) {
        /* Bank detected */
        while (j + i < strlen(str) && isalpha(str[j + i])) j++;
...
static int wt_type10_comp_decode(struct wt_type10_comp* comp,
    ...
    switch (specifier) {
        ...
        case COMP_SPEC_WVFRM:
            ...
            if (section->wvfrm.duration != 0) section->flags |= WT_TYPE10_COMP_DURATION_FLAG;
            break;
...
static int wt_type10_comp_str_to_bin(char* full_str, uint8_t* data) {
    ...
    return wt_type10_comp_to_buffer(&comp, WT_TYPE12_PWLE_SINGLE_PACKED_MAX, data);
}
 ...
 *
 */
static int wt_type12_pwle_write(struct wt_type12_pwle* pwle, void* buf, int size) {
    struct dspmem_chunk ch = dspmem_chunk_create(buf, size);
...
static int wt_type12_pwle_str_to_bin(char* full_str, uint8_t* data) {
    ...
    str = strtok(full_str, delim);
    while (str != NULL) {
        if (num_vals >= WT_TYPE12_PWLE_TOTAL_VALS) return -E2BIG;
        ...
        switch (wt_type12_pwle_specifier_get(strsep(&str, ":"))) {
            ...
            case PWLE_SPEC_CHIRP:
                ...
                if (val) section->flags |= WT_TYPE12_PWLE_CHIRP_BIT;
            ...
            case PWLE_SPEC_BRAKE:
                ...
                if (val) section->flags |= WT_TYPE12_PWLE_BRAKE_BIT;
    ...
    if (indef) pwle.wlength |= WT_INDEFINITE;
