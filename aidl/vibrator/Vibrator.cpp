...
static std::map<Effect, std::pair<short, int>> CP_TRIGGER_EFFECTS{
        {Effect::CLICK, {10, 10}}, {Effect::TEXTURE_TICK, {50, 5}}, {Effect::TICK, {50, 5}}};
...
ndk::ScopedAStatus Vibrator::getCompositionSizeMax(int32_t* _aidl_return) {
    ...
    *_aidl_return = (mUsesCommonFFInterface ? COMMON_MAX_COMPOSE_EFFECT / 2 : WT_MAX_SECTIONS / 2);
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus Vibrator::compose(const std::vector<CompositeEffect>& composite,
    ...
    if (!mSupportsPrimitives)
        return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
    if (composite.empty() ||
        (mUsesCommonFFInterface && composite.size() > COMMON_MAX_COMPOSE_EFFECT / 2) ||
        (!mUsesCommonFFInterface && composite.size() > WT_MAX_SECTIONS / 2))
        return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);

    int len, totalDuration = 0;
    ...
    for (auto segment : composite) {
        if (segment.delayMs < 0 ||
            (mUsesCommonFFInterface && segment.delayMs > COMMON_MAX_COMPOSE_DELAY) ||
            (!mUsesCommonFFInterface && segment.delayMs > WT_MAX_DELAY)) {
            return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
            ...
            if (segment.primitive == CompositePrimitive::NOOP) {
                ...
                if (segment.delayMs > 0) {
                    if (mUsesCommonFFInterface)
                        input_compose.effects[input_compose.num_of_effects++] = {
            ...
            totalDuration += duration + segment.delayMs;
            ...
            if (mUsesCommonFFInterface) {
                ...
                input_compose.effects[input_compose.num_of_effects++] = {FF_PERIODIC, (int)id,
                                                                         scale, duration, 0};
            } else {  // Cirrus OWT
                effect_str.append(segment.delayMs == 0
                                          ? std::format("{}.{},", cirrusEffectId(id), scale)
                                          : std::format("{} {}.{},", segment.delayMs,
                                                        cirrusEffectId(id), scale));
            }
        }

        setAmplitude(1);
        if (mUsesCommonFFInterface) {
            effectData.assign((int16_t*)&input_compose,
            ...
            effectData.assign(data, data + len);
        }
