...
ndk::ScopedAStatus Vibrator::getCompositionSizeMax(int32_t* _aidl_return) {
    ...
    *_aidl_return = MAX_COMPOSE_EFFECT;
    return ndk::ScopedAStatus::ok();
}
...
ndk::ScopedAStatus Vibrator::compose(const std::vector<CompositeEffect>& composite,
    ...
    if (!mSupportsPrimitives)
        return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
    if (composite.empty() || composite.size() > MAX_COMPOSE_EFFECT)
    ...
    for (auto segment : composite) {
        if (segment.delayMs < 0 || segment.delayMs > WT_MAX_DELAY)
            return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
        ...
        if (mUsesCommonFFInterface) {
            ...
            effect_str.append(
                    segment.delayMs == 0
                            ? std::format("{}.{}.{},", cirrusEffectId(id), scale, duration)
                            : std::format("{} {}.{}.{},", segment.delayMs, cirrusEffectId(id),
                                          scale, duration));
        }
    }
...
<<<<<<< PATCH SET (cec22c5962ab02466ad0b85d9910badde3c7488c aidl: vibrator: Add support for composite effects)
...
=======
>>>>>>> BASE      (61a816fae9f29034ad025bf2c4ca1aca589c33c5 aidl: vibrator: Return FF_EFFECT_IDS for FF vibrators)
#ifdef VIBRATOR_SUPPORTS_DURATION_AMPLITUDE_CONTROL
