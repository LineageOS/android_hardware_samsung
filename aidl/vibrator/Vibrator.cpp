...
ndk::ScopedAStatus Vibrator::perform(Effect effect, EffectStrength strength, const std::shared_ptr<IVibratorCallback>& callback, int32_t* _aidl_return) {
    ...
    if (!mHasTimedOutIntensity || !mHasTimedOutEffect)
        return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);

    activate(0);

    if (CP_TRIGGER_EFFECTS.find(effect) == CP_TRIGGER_EFFECTS.end())
        return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
...
ndk::ScopedAStatus Vibrator::getSupportedEffects(std::vector<Effect>* _aidl_return) {
    *_aidl_return = {};
    ...
    if (mHasTimedOutEffect) {
        for (auto entry : CP_TRIGGER_EFFECTS) {
            _aidl_return->push_back(entry.first);
        }
    }
