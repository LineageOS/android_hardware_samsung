...
static std::map<CompositePrimitive, HapticEffect> FF_PRIMITIVE_IDS{
...
ndk::ScopedAStatus Vibrator::perform(Effect effect, EffectStrength strength,
    ...
    if (mHasTimedOutEffect && CP_TRIGGER_EFFECTS.find(effect) != CP_TRIGGER_EFFECTS.end()) {
        ...
        if (mUsesCommonFFInterface) {
            uploadFFEffect({FF_EFFECT_IDS.at(effect).id}, 0);
        } else {
            uploadFFEffect({0, FF_EFFECT_IDS.at(effect).id}, 0);
        }
        ms = FF_EFFECT_IDS.at(effect).durationMs;
    } else {
        if (mHasTimedOutEffect) {
...
ndk::ScopedAStatus Vibrator::compose(const std::vector<CompositeEffect>& composite,
    ...
    for (auto segment : composite) {
        ...
        uint32_t id = FF_PRIMITIVE_IDS.at(segment.primitive).id;
