...
int main() {
    ...
    std::shared_ptr<AdaptiveBacklight> adaptiveBacklight = ndk::SharedRefBase::make<AdaptiveBacklight>();
    std::shared_ptr<DisplayColorCalibrationExynos> displayColorCalibration = ndk::SharedRefBase::make<DisplayColorCalibrationExynos>();
    std::shared_ptr<DisplayModes> displayModes = ndk::SharedRefBase::make<DisplayModes>();
    std::shared_ptr<ReadingEnhancement> readingEnhancement = ndk::SharedRefBase::make<ReadingEnhancement>();
    std::shared_ptr<SunlightEnhancementExynos> sunlightEnhancement = ndk::SharedRefBase::make<SunlightEnhancementExynos>();
    ...
    if (displayColorCalibration->isSupported()) {
        ...
        status = AServiceManager_addService(displayColorCalibration->asBinder().get(), instance.c_str());
        if (status != STATUS_OK) {
            LOG(ERROR)
