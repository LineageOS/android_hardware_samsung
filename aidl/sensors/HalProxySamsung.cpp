...
void HalProxySamsung::startProximityReactivationWorkerLocked(int32_t sensorHandle) {
    ...
    mProximityWorker =
            std::thread(&HalProxySamsung::proximityReactivationWorker, this, sensorHandle);
}
...
void HalProxySamsung::proximityReactivationWorker(int32_t sensorHandle) {
    while (true) {
...

        {
            ...
            if (mProximityWorkerCond.wait_for(
                        lock, std::chrono::milliseconds(intervalMs),
                        [this]() { return mProximityWorkerStop || !mStkProximityEnabled; })) {
                continue;
            }
        ...
        if (offResult != Result::OK || onResult != Result::OK) {
            ALOGW("STK proximity reactivation returned off=%d on=%d", static_cast<int>(offResult),
                  static_cast<int>(onResult));
        }
    }
}
