...
constexpr int SCREEN_OFF_PRESS_DELAY_MS = 0;
...
ndk::ScopedAStatus Session::onPointerDownWithContext(const PointerContext& context) {
    int screenOffPressDelayMs =
            FingerprintHalProperties::screen_off_press_delay().value_or(SCREEN_OFF_PRESS_DELAY_MS);

    if (screenOffPressDelayMs > SCREEN_OFF_PRESS_DELAY_MS) {
