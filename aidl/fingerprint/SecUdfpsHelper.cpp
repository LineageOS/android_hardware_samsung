...
int SecUdfpsHelper::setFodEnable(int i, int i2, int i3) {
    ...
    return setProperty(static_cast<int>(SemInputDevice::DEFAULT_TSP),
                       static_cast<int>(SemInputCommand::FOD), oss.str());
}

int SecUdfpsHelper::setFodIconVisible(int i) {
    return setProperty(static_cast<int>(SemInputDevice::DEFAULT_TSP),
                       static_cast<int>(SemInputCommand::FOD_ICON_VISIBLE), std::to_string(i) + "");
}

int SecUdfpsHelper::setFodLpMode(int i) {
    return setProperty(static_cast<int>(SemInputDevice::DEFAULT_TSP),
                       static_cast<int>(SemInputCommand::FOD_LP), std::to_string(i) + "");
}

int SecUdfpsHelper::setFodRect(const std::string& fodRect) {
    return setProperty(static_cast<int>(SemInputDevice::DEFAULT_TSP),
                       static_cast<int>(SemInputCommand::FOD_RECT), fodRect);
}

// Currently, calculating FodRect is complicated. Request original coordinates from stock OneUI
// temporarily as a FP HAL property.
/*
int SecUdfpsHelper::setFodRect(int i, int i2, int i3, int i4) {
    std::ostringstream oss;
    oss << i << "," << i2 << "," << i3 << "," << i4;
    return setProperty(static_cast<int>(SemInputDevice::DEFAULT_TSP),
static_cast<int>(SemInputCommand::FOD_RECT), oss.str());
}
*/
...
void SecUdfpsHelper::setFodRect(const std::string& fodRect) {
    ...
    while (getline(file, line)) {
        if (line == "set_fod_rect") {
            ...
            if (!fodRect.empty()) {
                ...
                LOG(INFO) << UDFPS_TAG
                          << "Rectangle FOD location is not defined, skipping setFodRect...";
            }
            return;
        }
    }
    ...
    if (!cmd_support) {
        LOG(INFO) << UDFPS_TAG
                  << "set_fod_rect command is not available to TSP, skipping setFodRect...";
    }
}
#endif
        ...
        if (res > 0 && (pfd.revents & POLLIN)) {
            ...
            if (value == "1" && lastValue != "1") {
                ...
                if (brightIn.is_open()) {
                    ...
                    LOG(WARNING) << UDFPS_TAG
                                 << "Kernel does not provide an HBM brightness level. Using "
                                    "default brightness: "
                                 << brightness;
                }
