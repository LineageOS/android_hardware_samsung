...
public class SoundBoosterStageService extends Service {
        ...
        try {
            ...
            if (devices != null) {
                for (AudioDeviceInfo dev : devices) {
                    if (isExternalDevice(dev.getType())) {
                        mBypassReason = "connected device " + deviceTypeName(dev.getType());
                        return false;
                    }
                }
            }
        ...
        try {
            ...
            if (configs != null && !configs.isEmpty()) {
                for (AudioPlaybackConfiguration cfg : configs) {
                    ...
                    if (outDevice != null && isExternalDevice(outDevice.getType())) {
                    ...
                    if (outDevice != null
                            ...
                            && outDevice.getType() != AudioDeviceInfo.TYPE_BUILTIN_SPEAKER_SAFE) {
                        mBypassReason = "active device " + deviceTypeName(outDevice.getType());
                        return false;
                    }
                }
    ...
    private static boolean isExternalDevice(int type) {
        switch (type) {
    ...
    private static boolean isEffectTypeAvailable(UUID type) {
        try {
            return AudioEffect.isEffectTypeAvailable(type);
        } catch (Exception ignored) {
            try {
