...
public final class MaskView implements DisplayBrightnessMonitor.OnBrightnessListener {
    ...
    private WindowManager.LayoutParams buildLayoutParams() {
        ...
        lp.flags &= ~FLAG_LAYOUT_INSET_DECOR;
        lp.privateFlags =
...
//      lp.gravity = Gravity.T    OP | Gravity.START;
    ...
    public void updateBackgroundColor(int displayBrightness) {
        ...
        float alpha = mDisplaySolution.getAlphaMaskLevel(displayBrightness, mDisplayBrightnessMonitor.mUdfpsBrightness, BRIGHTNESS_CONTROL_FACTOR);
        int maskAlpha = (int) (255f * alpha);
        int argb = (maskAlpha << 24);

        Log.i(TAG, "updateBackgroundColor: " + Integer.toHexString(argb) + ", " + mDisplayBrightnessMonitor.mUdfpsBrightness + ", " + maskAlpha);
