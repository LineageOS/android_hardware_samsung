...
int blToInt (const char *bootloader, bool ignore_last_chars = false) {
    int out = 0;
    size_t len = strlen(bootloader);
...
Value * CompareBootloaderFn(const char *name, State *state,
    ...
    for (auto& bl_version : args) {
        ...
        if (blToInt(bl_version.c_str(), true) >= blToInt(bootloader.c_str(), true)) {
