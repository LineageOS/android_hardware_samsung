 ...
 * @return 0 on success, -1 or errno on error.
 */
static int es_route_value_set(long value)
{
...
/*
 * Controls the power state of the Audience IC.
 *
 * @param value The power state.
 * @return 0 on success, -1 on error.
 */
static int es_power_control(int value)
{
	int ret;

    if (value < 0 || value > ES_MAX) {
        ALOGE("%s: unknown power mode: %d", __func__, value);
        return -1;
    }

    ret = write_int(SYSFS_PATH_POWERCTRL, value);
    /* arbitrary firmware load delay */
    usleep(2);
...
int es_start_voice_session(struct voice_session *session)
{
    ...
    /* make sure firmware is loaded before doing anything */
    es_power_control(ES_POWER_AWAKE);

    /*
     * The control flow for audience earSmart chip is as follows:
     * 
     * extra_volume >> route_value >> veq_control
     */
...
{
    /* ES_POWER_SLEEP is 1, but we actually want 0 to sleep */
    es_power_control(ES_POWER_SLEEP-1);
}
