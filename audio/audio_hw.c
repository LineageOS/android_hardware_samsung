...
static const char * const device_table[SND_DEVICE_MAX] = {
    ...
    [SND_DEVICE_OUT_VOICE_EARPIECE_WIDEBAND] = "voice-earpiece-wideband",
    [SND_DEVICE_OUT_VOICE_SPEAKER] = "voice-speaker",
    [SND_DEVICE_OUT_VOICE_SPEAKER_WIDEBAND] = "voice-speaker-wideband",
    [SND_DEVICE_OUT_VOICE_HEADPHONES] = "voice-headphones",
    [SND_DEVICE_OUT_VOICE_HEADPHONES_WIDEBAND] = "voice-headphones-wideband",
    ...
    [SND_DEVICE_IN_VOICE_EARPIECE_MIC_WIDEBAND] = "voice-earpiece-mic-wideband",
    [SND_DEVICE_IN_VOICE_SPEAKER_MIC] = "voice-speaker-mic",
    [SND_DEVICE_IN_VOICE_SPEAKER_MIC_WIDEBAND] = "voice-speaker-mic-wideband",
    [SND_DEVICE_IN_VOICE_HEADSET_MIC] = "voice-headset-mic",
    [SND_DEVICE_IN_VOICE_HEADSET_MIC_WIDEBAND] = "voice-headset-mic-wideband",
    ...
    if (mode == AUDIO_MODE_IN_CALL) {
        if (voice_session_uses_wideband(adev->voice.session)) {
            if (devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE ||
                ...
                snd_device = SND_DEVICE_OUT_VOICE_HEADPHONES_WIDEBAND;
            } else if (devices & AUDIO_DEVICE_OUT_SPEAKER) {
                snd_device = SND_DEVICE_OUT_VOICE_SPEAKER_WIDEBAND;
            } else if (devices & AUDIO_DEVICE_OUT_EARPIECE) {
                snd_device = SND_DEVICE_OUT_VOICE_EARPIECE_WIDEBAND;
            }
        } else {
            if (devices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE ||
                devices & AUDIO_DEVICE_OUT_WIRED_HEADSET) {
...
static snd_device_t get_input_snd_device(struct audio_device *adev, audio_devices_t out_device)
{
    ...
    if (mode == AUDIO_MODE_IN_CALL) {
        ...
        if (out_device & AUDIO_DEVICE_OUT_WIRED_HEADSET) {
            if (voice_session_uses_wideband(adev->voice.session)) {
                snd_device = SND_DEVICE_IN_VOICE_HEADSET_MIC_WIDEBAND;
            } else {
        ...
        if (voice_session_uses_twomic(adev->voice.session)) {
            if (voice_session_uses_wideband(adev->voice.session)) {
                if (out_device & AUDIO_DEVICE_OUT_EARPIECE ||
                    ...
                    snd_device = SND_DEVICE_IN_VOICE_EARPIECE_MIC_WIDEBAND;
                } else if (out_device & AUDIO_DEVICE_OUT_SPEAKER) {
                    snd_device = SND_DEVICE_IN_VOICE_SPEAKER_MIC_WIDEBAND;
                }
            } else {
                if (out_device & AUDIO_DEVICE_OUT_EARPIECE ||
                    out_device & AUDIO_DEVICE_OUT_WIRED_HEADPHONE) {
    ...
    if (out->usecase == USECASE_AUDIO_PLAYBACK_OFFLOAD) {
        ...
        if (!list_empty(&out->pcm_dev_list)) {
            ...
            if (pcm_device->pcm != NULL) {
                ret = pcm_get_htimestamp(pcm_device->pcm, &avail, timestamp);
                if (ret == 0) {
                ...
                /*
                 * This happens after a phone call. We have our voice pcm_config
                 * in voice.c so it isn't open here. It is also possible that
                 * we need to walk the list and find the first pcm_device with
                 * the pcm not being NULL.
                 */
                ALOGE("%s: No PCM open for stream, check FIXME in the code",
                      __func__);
            }
        }
    }
