...
#define WIFI_MODULE_NAME   "wlan"
#define WIFI_MODULE_PATH   "/system/lib/modules/" WIFI_MODULE_NAME ".ko"
...
static int load_module(char const *path)
{
    ...
    if (check_module_loaded(WIFI_MODULE_NAME) > 0) {
...
int main(void)
{
    ...
    if (stat(WIFI_MODULE_PATH, &st) == 0) {
        ALOGD("Loading WiFi kernel module: %s", WIFI_MODULE_PATH);
        load_module(WIFI_MODULE_PATH);
    }
