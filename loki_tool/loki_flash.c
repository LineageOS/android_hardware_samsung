...
int loki_flash(const char* partition_label, const char* loki_image) {
    ...
    for (offs = 0; offs < 0x10; offs += 0x4) {
        ...
        if (!memcmp(patch, PATTERN1, 8) || !memcmp(patch, PATTERN2, 8) ||
            !memcmp(patch, PATTERN3, 8) || !memcmp(patch, PATTERN4, 8) ||
            !memcmp(patch, PATTERN5, 8) || !memcmp(patch, PATTERN6, 8)) {
            match = 1;
            break;
        }
    }
    ...
    snprintf(outfile, sizeof(outfile), "%s", recovery ? RECOVERY_PARTITION : BOOT_PARTITION);
