...
int patch_shellcode(unsigned int header, unsigned int ramdisk) {
    ...
    if (found_header && found_ramdisk) return 0;
...
int loki_patch(const char* partition_label, const char* aboot_image, const char* in_image,
    ...
    unsigned int orig_ramdisk_size, orig_kernel_size, page_kernel_size, page_ramdisk_size,
            page_size, page_mask;
    ...
    for (ptr = aboot; ptr < aboot + st.st_size - 0x1000; ptr++) {
        if (!memcmp(ptr, PATTERN1, 8) || !memcmp(ptr, PATTERN2, 8) || !memcmp(ptr, PATTERN3, 8) ||
            !memcmp(ptr, PATTERN4, 8) || !memcmp(ptr, PATTERN5, 8)) {
    ...
    orig = mmap(0, (st.st_size + 0x2000 + 0xfff) & ~0xfff, PROT_READ | PROT_WRITE, MAP_PRIVATE, ifd,
                0);
    if (orig == MAP_FAILED) {
    ...
    if (hdr->dt_size) {
        ...
        if (write(ofd, orig + page_size + page_kernel_size + page_ramdisk_size, hdr->dt_size) !=
            hdr->dt_size) {
