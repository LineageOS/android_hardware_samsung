...
class BootCompletedReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val dapEnabled = sharedPrefs.getBoolean(PREF_DOLBY_ENABLE, false)
        if (dapEnabled) {
            val dapMode = sharedPrefs.getString(PREF_DOLBY_MODES, "0")!!.toInt()
            DolbyCore().startDolbyEffect(dapMode)
        }
    }
}
