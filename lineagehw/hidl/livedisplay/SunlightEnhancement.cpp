...
bool SunlightEnhancement::isSupported() {
    std::fstream fileSRE(kSREPath, fileSRE.in | fileSRE.out);
    std::fstream fileHBM(kHBMPath, fileHBM.in | fileHBM.out);
    return fileSRE.good() || fileHBM.good();
}

// Methods from ::vendor::lineage::livedisplay::V2_0::IAdaptiveBacklight follow.
Return<bool> SunlightEnhancement::isEnabled() {
    std::ifstream fileSRE(kSREPath);
    std::ifstream fileHBM(kHBMPath);
    int statusSRE = -1;
    int statusHBM = -1;
...
Return<bool> SunlightEnhancement::setEnabled(bool enabled) {
    std::ofstream fileSRE(kSREPath);
    std::ofstream fileHBM(kHBMPath);
